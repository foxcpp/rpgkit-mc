...
import com.github.foxcpp.rpgkitmc.magic.items.MagicChannelingItem;
...
public class ActiveCastComponent implements ComponentV3, AutoSyncedComponent, ClientTickingComponent, ServerTickingComponent {
    ...
    public void startBuild() {
        ...
        if (this.provider.getComponent(ModComponents.MANA).getValue() == 0) {
            this.provider.sendMessage(Text.translatable("rpgkit.magic.must_have_mana"), true);
            return;
        }
        ...
        if (!(this.provider.getMainHandStack().getItem() instanceof MagicChannelingItem)) {
            if (!this.provider.getMainHandStack().isEmpty()) {
                this.provider.sendMessage(Text.translatable("rpgkit.magic.must_have_empty_hand"), true);
                return;
            }
    ...
    public ActionResult performCastOnBlock(BlockPos pos, Direction direction) {
        ...
        if (pos.getSquaredDistance(this.provider.getEyePos().x, this.provider.getEyePos().y, this.provider.getEyePos().z) > 36) {
            RPGKitMagicMod.LOGGER.warn("Player {} sent CastType.USE_BLOCK for a block more than 6 blocks away, ignoring and canceling spell", this.provider);
            this.cancelBuild();
            return ActionResult.FAIL;
        }
    ...
    public ActionResult performCastOnEntity(Entity target) {
        ...
        if (target.getPos().squaredDistanceTo(this.provider.getEyePos()) > 36) {
            RPGKitMagicMod.LOGGER.warn("Player {} sent CastType.USE_ENTITY for an entity more than 6 blocks away, ignoring and canceling spell", this.provider);
            this.cancelBuild();
            return ActionResult.FAIL;
        }
    ...
    public void serverTick() {
        ...
        if (this.isChanneling()) {
            if (!(cast.getForm() instanceof ChanneledForm cf)) {
                RPGKitMagicMod.LOGGER.error("Cast form for {} is not channelable, but channelMaxAge is set, cancelling cast.", this.provider);
                this.endCast();
                return;
            }
