...
public class SpellCast {
    ...
    protected SpellCast(NbtCompound nbt) {
        ...
        Spell spell;
        try {
            var spellJson = RPGKitMod.GSON.fromJson(nbt.getString("SpellData"), JsonObject.class);
            spell = new Spell(spellJson);
        } catch (JsonSyntaxException | IllegalArgumentException ex) {
            RPGKitMod.LOGGER.error("Invalid SpellData, replacing with empty spell", ex);
            spell = Spell.EMPTY;
        }
    ...
    public void writeToNbt(NbtCompound nbt) {
        ...
        nbt.putString("SpellData", RPGKitMod.GSON.toJson(spellJson));
